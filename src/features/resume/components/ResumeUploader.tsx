...
export default function ResumeUploader() {
  ...
  function handleFileChange(file: File) {
    ...
    if (file.type.split("/")[0] !== "application") {
      toast.error("We only accept pdf files");
      return;
    }
    ...
    <section className="mb-[1rem]">
      <div>
        <label
          ...
          className="group relative mt-2 flex h-40 cursor-pointer flex-col items-center justify-center rounded-md border-[1px] border-white bg-white shadow-sm transition-all hover:bg-gray-50 md:h-32"
        >
          <div
        ...
        <div className="mt-1 flex rounded-md shadow-sm">
          <input
            ...
            name="image"
            type="file"
            accept="application/pdf" // Only allow PDFs         multiple // Allow multiple files
            className="sr-only"
            onChange={(event) => {
      ...
      <div className="space-y-2">
        ...
        {files.length > 0 && (
          <button
            ...
            className="mt-[1rem] flex h-10 w-full items-center justify-center rounded-md border border-gray-200 bg-gray-100 text-sm text-gray-700 transition-all duration-300 cursor-pointer ease-in-out hover:bg-white hover:text-black focus:outline-none disabled:cursor-not-allowed disabled:border-gray-200 disabled:bg-gray-100 disabled:text-gray-400"
          >
            Reset
          </button>
        )}
